...
- Auth.css's `.form-section` is dead as of 9f. DiscountForm was the only thing it ever matched, and Auth renders `auth-form-section` instead.
- Rules no component uses, renamed in 9f: `.discount-form-amount-input`, `.discount-form-gradient-button` and `.discount-form-block-button` in DiscountForm.css.
- DiscountForm.js sets `discount-form-message-section` (formerly `form-section2`, next to a `form-section` that did have rules), and no stylesheet has a rule for it.
- App.css's opening `#root, .App` rule is empty: every declaration inside it is commented out.
